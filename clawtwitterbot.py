...
def getlog():
    try:
        ...
        print("Connected to ICON API")   
    except:
        print("[bright_yellow]Timeout to ICON API[/bright_yellow]")
...
def gettxresult():
    ...
    if status != "ICXTransfer(Address,Address,int)":
        ...
        try:
            ...
            print("[bright_yellow]Couldnt get TokenID from TX[/bright_yellow]")
            print("[bright_yellow]data in eventlog: [/bright_yellow]", data)
            for step in track(range(100), description="Checking again soon"):
                time.sleep(0.2)
            gettxresult()
        ...
        table2.add_column("Royalty:", style="dim")
        ...
        table2.add_column("Royalty Balance:", justify="right")
        table2.add_row(
        stramount, roystramount, strtokenid, url, strrank, status, strbalance
        ) 
...
def check_old():
    try:
        ...
        current_time = now.strftime("%H:%M:%S")
        print(current_time + " couldnt read icon API")
        for step in track(range(100), description="Checking again soon"):
                time.sleep(0.2)
        check_old()
    else:
